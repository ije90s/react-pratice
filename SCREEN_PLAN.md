...
### Phase 5 — 상세/삭제 마무리
- [x] `/feeds/:feedId` — `GET/DELETE` (피드 응답에 `user_id`가 있어 `useMe()`로 "내 글일 때만 수정·삭제" 판단 가능, 백엔드 수정 불필요)
...
- [x] 상세 화면에 "수정" 버튼 — 내 글일 때만 노출(챌린지·피드 상세 모두 완료). `GET /user/me`로 내 `id`를 받아 `challenge.author_id`와 비교해야 해서 Phase 5(삭제 버튼)와 함께 처리. 지금은 `/challenges/:id/edit`를 주소창으로만 접근 가능(남의 글은 서버가 403)
